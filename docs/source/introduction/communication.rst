...
Here's a simple example::
    ...
    >>> from lecroydso 
    >>> dso = lecroydso.ResourceManager()
    >>> rm.list_resources()
    ('ASRL1::INSTR', 'ASRL2::INSTR', 'GPIB0::14::INSTR')
    >>> my_instrument = rm.open_resource('GPIB0::14::INSTR')
    >>> print(my_instrument.query('*IDN?'))

This example already shows the two main design goals of LeCroyDSO: preferring
simplicity over generality, and doing it the object-oriented way.

After importing ``lecroydso``, we create a ``ResourceManager`` object. If called
without arguments, LeCroyDSO will prefer the default backend (IVI) which tries to
find the VISA shared library for you. If it fails it will fall back to
lecroydso-py if installed. You can check what backend is used and the location of
...
.. note::
    ...
    In some cases, LeCroyDSO is not able to find the library for you resulting in
...
You can configure LeCroyDSO to communicate to your instrument using a different
...
LeCroyDSO `issue tracker`_. If you do be sure to describe in detail your setup and
...
The above focused on using only LeCroyDSO,  if you are running Windows, or MacOS
...
LeCroyDSO. However if it does not work using those tools, it won't work in
LeCroyDSO.
